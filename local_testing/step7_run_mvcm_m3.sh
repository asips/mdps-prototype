...
collection_id=CLDMSK_M3_VIIRS_NOAA20
...
mkdir -p $protodir/local_testing/mvcm_m3-inputs/$collection_id/
cd $protodir/local_testing/mvcm_m3-inputs/$collection_id/

rsync -av $protodir/local_testing/mvcm_d3-outputs/*/CLDMSK*.nc .
...
cd $protodir
cwltool \
    --outdir=./local_testing/mvcm_m3-outputs/ \
    $protodir/workflows/mvcm_l3/tasks/process_m3.cwl \
    $protodir/local_testing/mvcm_m3-inputs/$collection_id.yaml
